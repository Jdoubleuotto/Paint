...
let list_layout (pair: widget -> widget -> widget) (ws: widget list) : widget =
    List.fold_right (fun a b -> pair a b) ws (pair (space(0,0)) (space(0,0)))
...
let make_controller (v: 'a) : 'a value_controller =
  ...
  add_change_listener = (fun x -> listeners.contents <- x::listeners.contents);
   ...
   change_value = (fun (s : 'a) -> r.contents <- s; 
                   List.iter (fun v -> v s) listeners.contents;)
  }
...
let checkbox (init: bool) (s: string) : widget * bool value_controller =
  ...
  let name, _ = label s in 
  let (w, h) = name.size() in 
 ({
 (* draw a blank rectangle and the text and if box is clicked then fill it in*)
    repaint = (fun (g: Gctx.gctx) -> 
        name.repaint (Gctx.translate g (19, 0));
        if controller.get_value()
          then (Gctx.fill_rect g (2,4) (11,11);
                Gctx.draw_rect g (0,2) (15,15))
          else Gctx.draw_rect g (0,2) (15,15));
    (*handle mouse clock events only*)
    handle = (fun (g: Gctx.gctx) (e: Gctx.event)  ->
        if (Gctx.event_type e = MouseDown) 
          then controller.change_value (not (controller.get_value()))
          else ());
    (*return the max of the y size becuase we are create a psuedo hpair*)
    size = (fun () -> (15 + w, max h 15))
  },
    controller
  )
...
(* TODO: In Task 6 you may choose to add a radio_button widget, a
  ...
  (* create a slider of type widget and int value controller that takes in the 
  an x value to determine the size of the slider*)
  let slider (v : int) (color : Gctx.color): widget * int value_controller = 
    let controller = make_controller v in 
    let (w,h) = (83,17) in 
  ({
  (* repaint the bar inside the slider depending on where the mouse is*)
    repaint = (fun (g :Gctx.gctx) -> Gctx.draw_rect g (0,0) (w,h);
                      Gctx.fill_rect (Gctx.with_color g color) (1,1) 
                      ((controller.get_value()), h - 2));
    (* if the user clicks or drags on the widget then update the size *)
    handle = (fun (g : Gctx.gctx) (e: Gctx.event) -> 
      if (Gctx.event_type e = MouseDown || Gctx.event_type e = MouseDrag) 
        then (controller.change_value (fst(Gctx.event_pos e g));)
        else ());
    (*simply returns the size of the slider*)
    size = (fun () -> (w,h))
  }, 
    controller
  )
